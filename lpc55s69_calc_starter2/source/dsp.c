...
header* maccel (Calc *cc, header *hd)
{
	...
	header *result = new_matrix(cc,col,row,"");
...
header* mpqcos(Calc* cc, header* hd) {
	const stacktyp scalaire = s_real;
	const stacktyp vectoriel = s_matrix;
	header* result;
	int col, row;
	real *m, *mr;
	...
	}else if(hd->type == vectoriel){

		result = new_matrix(cc, row, col, "");
		mr = matrixof(result);

		PQ_VectorCosF32(m, mr, row*col);
...
//		}
		for (int i = 0; i < col; i++) {
			mr[2*i] =  ((real)fft_out[2*i])/(1 << 9);
			mr[2*i + 1] =  ((real)fft_out[2*i + 1])/(1 << 9);
		}
